...
| ↓ кнопка | мгновенный сброс cooldown | `dragSettleDelay` 0.16s (ждём) | **наш** — их задержка заметна, но реализация их латча. Наш `post(.hermexCancelTranscriptInertia)` в `scrollToBottom` слияние потеряло → восстановлен |
...
| Список сессий: messaging | `!isMessagingSession` — скрывать | #320: показывать и открывать через импорт | **их** — наш фильтр был обходом 403, который #320 вылечил |
